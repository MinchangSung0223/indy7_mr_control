...
namespace mr {
	 ...
	 */
       Eigen::MatrixXd w_p_to_Slist(const Eigen::Vector3f *w, const Eigen::Vector3f *p,int ROBOT_DOF){
		// Minchang Sung
	...
	std::vector<Eigen::MatrixXd> getGlist(const Eigen::Matrix3f *inertia, float *mass,int ROBOT_DOF  ){
	...
	std::vector<Eigen::MatrixXd> getMlist(const Eigen::Vector3f *L,const Eigen::Vector3f *CoM,int ROBOT_DOF) {
		// Minchang Sung
		std::vector<Eigen::MatrixXd> Mlist;
		std::vector<Eigen::MatrixXd> Mlist_;
		Mlist.push_back(Eigen::MatrixXd::Identity(4,4));
		for(int i = 0;i<ROBOT_DOF-1;i++){
		...
		std::vector<Eigen::MatrixXd> getCoM_Mlist(const Eigen::Vector3f *CoM,int ROBOT_DOF) {
		// Minchang Sung
		std::vector<Eigen::MatrixXd> Mlist;
		std::vector<Eigen::MatrixXd> Mlist_;
		for(int i = 0;i<ROBOT_DOF+1;i++){
	...
	std::vector<Eigen::MatrixXd> getHTranslist(const Eigen::Vector3f *p,const Eigen::MatrixXd& M, const Eigen::MatrixXd& Slist, const Eigen::VectorXd& thetaList) {
		...
		std::vector<Eigen::MatrixXd> Mlist;	
		int ROBOT_DOF = thetaList.size();
		Eigen::Vector3f L[ROBOT_DOF];
		L[ROBOT_DOF-1] << M(0,3),M(1,3),M(2,3);
